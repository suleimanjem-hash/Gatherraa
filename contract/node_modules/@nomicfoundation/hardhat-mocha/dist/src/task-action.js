...
import { markTestRunStart as initCoverage, markTestWorkerDone as saveCoverageData, markTestRunDone as reportCoverage, } from "hardhat/internal/coverage";
import { markTestRunStart as initGasStats, markTestWorkerDone as saveGasStats, markTestRunDone as reportGasStats, } from "hardhat/internal/gas-analytics";
...
async function getTestFiles(testFiles, config) {
    ...
    return getAllFilesMatching(config.paths.tests.mocha, (f) => isJavascriptFile(f) || isTypescriptFile(f));
}
let testsAlreadyRun = false;
const testWithHardhat = async ({ testFiles, bail, grep, noCompile }, hre) => {
    ...
    if (hre.config.test.mocha.parallel === true) {
        ...
        if (hre.globalOptions.coverage === true) {
            const coverage = new URL(import.meta.resolve("@nomicfoundation/hardhat-mocha/coverage"));
            hre.config.test.mocha.require.push(coverage.href);
        }
        if (hre.globalOptions.gasStats === true) {
            const gasStats = new URL(import.meta.resolve("@nomicfoundation/hardhat-mocha/gas-stats"));
            hre.config.test.mocha.require = hre.config.test.mocha.require ?? [];
